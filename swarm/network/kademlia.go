...
func NewPeerPot(kadMinProxSize int, ids []discover.NodeID, addrs [][]byte) map[discover.NodeID]*PeerPot {
	...
	for i, id := range ids {
		...
		//		ppnns := make([][]byte, len(nns))
		//		for i := range ppnns {
		//			ppnns[i] = make([]byte, len(nns[i]))
		//			copy(ppnns[i], nns[i])
		//			log.Trace("newpeerpot", "i", i, "ppnns", fmt.Sprintf("%p", ppnns[i]), "nns", fmt.Sprintf("%p", nns[i]), "nn", fmt.Sprintf("%x", ppnns[i]))
		//		}
		//		ppmap[id] = &PeerPot{ppnns, emptyBins}
